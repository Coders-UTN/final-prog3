...
class ComponenteOrders {
    ...
    constructor() {
        ...
        this.selectEstado = document.getElementById("select-estado-pedido") as HTMLSelectElement;
    ...
    crearFilaPedido(order: IOrder): HTMLTableRowElement {
        ...
        row.innerHTML = `
            ...
            <td><span class="estado ${order.estado.toLowerCase()}">${order.estado}</span></td>
    ...
    manejarVerDetalle(idPedido: number): void {
        ...
        (document.getElementById("modal-titulo-pedido") as HTMLElement).textContent = `Detalle del Pedido #${pedido.id}`;

        this.listaItemsModal.innerHTML = "";
        pedido.detallePedidoDTO.forEach(item => {
            const li = document.createElement('li');
            li.textContent = `${item.cantidad}x ${item.nombreProducto} ($${item.precioUnitario.toFixed(2)} c/u)`;
    ...
    adjuntarEventosBase(): void {
        ...
        const closeBtn = document.querySelector("#modal-detalle-pedido .modal-cerrar");
        closeBtn?.addEventListener("click", () => this.cerrarModal());

        const cancelBtn = document.querySelector("#modal-detalle-pedido .btn-cancelar");
